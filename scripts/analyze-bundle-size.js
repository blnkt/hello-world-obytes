...
function getDirectorySize(dirPath, extensions = ['.ts', '.tsx', '.js', '.jsx']) {
  let totalSize = 0;
  let fileCount = 0;
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          if (!['node_modules', '.git', 'ios', 'android', 'coverage', '.expo'].includes(item)) {
...
function analyzeSourceCode() {
  ...
  const componentsStats = fs.existsSync(componentsPath) ? getDirectorySize(componentsPath) : { totalSize: 0, fileCount: 0 };
  const libStats = fs.existsSync(libPath) ? getDirectorySize(libPath) : { totalSize: 0, fileCount: 0 };
  const appStats = fs.existsSync(appPath) ? getDirectorySize(appPath) : { totalSize: 0, fileCount: 0 };
...
function generateReport() {
  ...
  console.log(`  Components: ${source.components.fileCount} files, ${formatBytes(source.components.totalSize)}`);
  console.log(`  Library code: ${source.lib.fileCount} files, ${formatBytes(source.lib.totalSize)}`);
  console.log(`  App code: ${source.app.fileCount} files, ${formatBytes(source.app.totalSize)}\n`);
  ...
  const estimatedBundleSize = source.total.totalSize + (mocks.totalSize * 0.1); // Mocks have minimal bundle impact
  console.log('📈 Estimated Bundle Impact:');
  console.log(`  Estimated bundle size: ${formatBytes(estimatedBundleSize)}`);
  console.log(`  Mock overhead: ${formatBytes(mocks.totalSize * 0.1)} (10% of mock size)`);
  console.log('');
