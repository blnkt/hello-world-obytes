...
function checkBundlePerformance(bundleAnalysis) {
  ...
  if (bundleAnalysis.components && bundleAnalysis.components.totalSize > PERFORMANCE_BUDGETS.bundle.components) {
    violations.push({
      category: 'bundle',
      metric: 'components',
  ...
  // Check dependencies
  if (bundleAnalysis.dependencies && bundleAnalysis.dependencies.totalDependencies > PERFORMANCE_BUDGETS.bundle.dependencies) {
    violations.push({
      category: 'bundle',
      metric: 'dependencies',
...
function checkTestPerformance(testAnalysis) {
  ...
  if (testAnalysis.mockSetupTime && testAnalysis.mockSetupTime > PERFORMANCE_BUDGETS.testing.mockSetup) {
    violations.push({
      category: 'testing',
      metric: 'mockSetup',
  ...
  if (testAnalysis.totalTime && testAnalysis.totalTime > PERFORMANCE_BUDGETS.testing.totalTestTime) {
    violations.push({
      category: 'testing',
      metric: 'totalTime',
...
function loadPerformanceData() {
  ...
  const bundleAnalysisPath = path.join(process.cwd(), 'bundle-analysis-report.json');
...
function generateReport(violations) {
  ...
  console.log(`❌ Found ${violations.length} performance budget violation(s):\n`);
