...
describe('ReturnCostIndicators (Task 1.6)', () => {
  it('should render return cost indicators', () => {
    const { getByTestId } = render(
      <ReturnCostIndicators currentEnergy={50} returnCost={30} depth={2} />
    );
  ...
  it('should show warning when safety margin is low', () => {
    const { getByTestId } = render(
      <ReturnCostIndicators currentEnergy={35} returnCost={30} depth={2} />
    );
  ...
  it('should indicate critical safety margin', () => {
    const { getByText } = render(
      <ReturnCostIndicators currentEnergy={25} returnCost={30} depth={2} />
    );
  ...
  it('should display depth information', () => {
    const { getAllByText } = render(
      <ReturnCostIndicators currentEnergy={50} returnCost={30} depth={3} />
    );
