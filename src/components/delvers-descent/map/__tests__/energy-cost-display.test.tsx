...
describe('EnergyCostDisplay (Task 1.5)', () => {
  it('should render energy cost display', () => {
    const { getByTestId } = render(
      <EnergyCostDisplay currentEnergy={50} totalEnergy={100} />
    );
  ...
  it('should show energy cost for an action', () => {
    const { getByText } = render(
      <EnergyCostDisplay currentEnergy={50} totalEnergy={100} actionCost={10} />
    );
  ...
  it('should indicate if action cost is affordable', () => {
    const { getByTestId } = render(
      <EnergyCostDisplay currentEnergy={50} totalEnergy={100} actionCost={30} />
    );
  ...
  it('should show warning when energy is low', () => {
    const { getByText } = render(
      <EnergyCostDisplay currentEnergy={10} totalEnergy={100} />
    );
  ...
  it('should show remaining energy after cost', () => {
    const { getByText } = render(
      <EnergyCostDisplay currentEnergy={50} totalEnergy={100} actionCost={20} />
    );
