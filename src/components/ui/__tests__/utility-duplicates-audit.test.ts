...
describe('Utility Duplicates Audit', () => {
  ...
  const utilityPatterns = [
    ...
    {
      pattern: /^formatDate$|^formatTime$|^formatDateTime$/,
      name: 'Date formatting',
    },
    { pattern: /parseDate|parseTime|parseDateTime/, name: 'Date parsing' },
    {
      pattern: /isValidDate|isValidTime|isValidDateTime/,
      name: 'Date validation',
    },

    // String utilities
    {
      pattern: /capitalize|toTitleCase|toUpperCase|toLowerCase/,
      name: 'String case conversion',
    },
    { pattern: /trim|strip|clean/, name: 'String cleaning' },
    {
      pattern: /slugify|kebabCase|camelCase|snakeCase/,
      name: 'String transformation',
    },

    // Number utilities
    {
      pattern: /formatNumber|formatCurrency|formatPercent/,
    ...
    // Storage utilities
    {
      pattern: /getStorage|setStorage|removeStorage/,
      ...
      try {
        ...
        utilityPatterns.forEach(({ pattern, name }) => {
          const matches = content.match(
            new RegExp(
              `(?:function|const|let|var)\\s+(${pattern.source})\\s*[=(]`,
              'g'
            )
          );
