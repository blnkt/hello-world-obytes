  ...
  it('should verify components use consistent color patterns', () => {
    ...
    componentFiles.forEach(filePath => {
      ...
      const nonCommentHardcodedColors = hardcodedColors.filter(color => {
        ...
        return lines.some(line => 
          line.includes(color) && !line.trim().startsWith('//') && !line.trim().startsWith('*')
        );
      });
      ...
      expect(content).toMatch(/bg-(charcoal|primary|success|danger|warning|neutral)-/);
    });
    ...
    allComponentFiles.forEach(filePath => {
      ...
      if (content.includes('import colors from') || 
          content.match(/bg-(charcoal|primary|success|danger|warning|neutral)-/) ||
          content.match(/text-(charcoal|primary|success|danger|warning|neutral)-/) ||
          content.match(/border-(charcoal|primary|success|danger|warning|neutral)-/)) {
        componentsUsingCentralizedColors++;
      }
    });

    // At least 75% of components should use centralized color system
    expect(componentsUsingCentralizedColors).toBeGreaterThanOrEqual(Math.ceil(allComponentFiles.length * 0.75));
  });
