...
export default function BustScreen() {
  ...
  const { handleAcknowledge } = useBustHandlers(router);
    ...
    <ScrollView className="flex-1 bg-red-50 dark:bg-red-900/20">
      <View className="p-6">
        <View className="mx-auto max-w-2xl">
          <BustHeader />
          <ConsequenceDetails consequence={consequence} />
          <ContinueButton onPress={handleAcknowledge} />
        </View>
      </View>
    </ScrollView>
  );
...
function useBustHandlers(router: ReturnType<typeof useRouter>) {
...
function ConsequenceDetails({ consequence }: { consequence: BustConsequence }) {
  return (
