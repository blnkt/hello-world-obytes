import { ShortcutManager, ShortcutInfo } from './shortcut-manager';
...
describe('ShortcutManager', () => {
  ...
  describe('shortcut discovery and management', () => {
    it('should discover new shortcuts', () => {
      ...
      expect(discoveredShortcut?.reductionFactor).toBe(shortcut.reductionFactor);
    ...
    it('should not allow duplicate shortcut discovery', () => {
      ...
      expect(shortcutManager.getShortcut('shortcut-1')).toEqual(originalShortcut);
    });

    it('should get all discovered shortcuts', () => {
      const shortcuts: ShortcutInfo[] = [
        { id: 'shortcut-1', depth: 2, reductionFactor: 0.7, description: 'Passage 1' },
        { id: 'shortcut-2', depth: 4, reductionFactor: 0.8, description: 'Passage 2' },
        { id: 'shortcut-3', depth: 3, reductionFactor: 0.6, description: 'Passage 3' }
      ];

      shortcuts.forEach(shortcut => shortcutManager.discoverShortcut(shortcut));
    ...
    it('should get shortcuts available at specific depth', () => {
      ...
      expect(availableAtDepth3.map(s => s.id)).toEqual(expect.arrayContaining(['shortcut-1', 'shortcut-3']));
    });
  });
  ...
  describe('shortcut effectiveness', () => {
    it('should calculate effective reduction factor for multiple shortcuts', () => {
      ...
      const shortcuts: ShortcutInfo[] = [
        { id: 'shortcut-1', depth: 2, reductionFactor: 0.7, description: 'Passage 1' },
        { id: 'shortcut-2', depth: 3, reductionFactor: 0.8, description: 'Passage 2' }
      ];

      shortcuts.forEach(shortcut => shortcutManager.discoverShortcut(shortcut));
      
      const effectiveReduction = shortcutManager.calculateEffectiveReduction(3, ['shortcut-1', 'shortcut-2']);
    ...
    it('should return 0 reduction when no shortcuts are available', () => {
      const effectiveReduction = shortcutManager.calculateEffectiveReduction(3, []);
    ...
    it('should handle shortcuts that are not applicable to current depth', () => {
      ...
      const effectiveReduction = shortcutManager.calculateEffectiveReduction(3, ['shortcut-1']);
      expect(effectiveReduction).toBe(0);
    });
  });
