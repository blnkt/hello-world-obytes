...
describe('Path Optimization Algorithm', () => {
  ...
  describe('path optimization with shortcuts', () => {
    it('should find optimal path using shortcuts', () => {
      ...
      const optimalCost = calculator.calculateOptimalReturnCost(
        currentDepth,
        visitedNodes,
        shortcutManager.getShortcutIdsAtDepth(currentDepth),
        shortcutManager.getAllShortcuts().reduce((map, shortcut) => {
    ...
    it('should consider visited nodes in path optimization', () => {
      ...
      const shortcuts: ShortcutInfo[] = [
        { id: 'shortcut-1', depth: 2, reductionFactor: 0.7, description: 'Passage 1' },
        { id: 'shortcut-2', depth: 3, reductionFactor: 0.6, description: 'Passage 2' }
      ];
      shortcuts.forEach(shortcut => shortcutManager.discoverShortcut(shortcut));
    ...
    it('should handle multiple shortcuts at same depth', () => {
      ...
      const shortcuts: ShortcutInfo[] = [
        { id: 'shortcut-1', depth: 2, reductionFactor: 0.7, description: 'Passage 1' },
        { id: 'shortcut-2', depth: 2, reductionFactor: 0.8, description: 'Passage 2' }
      ];
      shortcuts.forEach(shortcut => shortcutManager.discoverShortcut(shortcut));
  ...
  describe('path optimization edge cases', () => {
    ...
    it('should handle shortcuts beyond current depth', () => {
      const shortcuts: ShortcutInfo[] = [
        { id: 'shortcut-1', depth: 5, reductionFactor: 0.7, description: 'Deep passage' }
      ];
      shortcuts.forEach(shortcut => shortcutManager.discoverShortcut(shortcut));
  ...
  describe('performance requirements', () => {
    it('should complete path optimization within performance requirements', () => {
      ...
      const visitedNodes = Array.from({ length: 10 }, (_, i) => `node-${i + 1}`);
  ...
  describe('integration with existing systems', () => {
    it('should integrate with shortcut manager seamlessly', () => {
      ...
      expect(optimalCost).toBeGreaterThan(0);
      expect(optimalCost).toBeLessThan(calculator.calculateCumulativeReturnCost(currentDepth));
    });
  });
});
